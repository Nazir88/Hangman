...
const main = () => {
    const wordBank = ["JAVSCRIPT", "CODE", "BOOLEAN", "INTEGER", "STRING", "ARRAY"];
    let wordToGuess = generateWord(wordBank);
    let attemptsRemaining = 6;
    let correctAttempts = new Array(wordToGuess.length).fill('_ '); 
    let attempted = [];
    let all = ['o', '|', '-', '-', '/', '\\'];
    let bp = [' ', ' ', ' ', ' ', ' ', ' '];

    alert('Welcome to worlds best hangman game!!!');
